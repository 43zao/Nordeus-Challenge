...
public class GameState : MonoBehaviour
{
    ...
    public void ApplyStatUpgrade(string stat)
    {
        ...
        {
            case "attack":
                currentRun.hero.stats.attack += 1;
                break;

            case "defense":
                currentRun.hero.stats.defense += 1;
                break;

            case "magic":
                currentRun.hero.stats.magic += 1;
                break;

            case "hp":
                currentRun.hero.stats.hp += 10;
                break;
        }
    ...
    public void TryLearnRandomMonsterMove(MonsterData monster)
    {
        ...
        foreach (var move in monster.moves)
        {
            ...
            foreach (var known in knownMoves)
            {
                if (known.name == move.name)
                {
                    alreadyKnown = true;
                    break;
                }
        ...
        {
            Debug.Log("No new moves to learn.");
            return;
        }

        MoveData learnedMove =
            availableMoves[UnityEngine.Random.Range(0, availableMoves.Count)];

        knownMoves.Add(learnedMove);

        Debug.Log("Learned new move: " + learnedMove.name);
    }
    */
